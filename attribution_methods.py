...
def get_attributor(
    ...
    return attributor_map[attributor_name](
        model, only_positive, binarize, interpolate, interpolate_dims, batch_mode
    )
    ...
    def check_interpolate(self, attributions):
        if self.interpolate:
            return captum.attr.LayerAttribution.interpolate(
                attributions,
                interpolate_dims=self.interpolate_dims,
                interpolate_mode="bilinear",
            )
        return attributions

    def check_binarize(self, attributions):
        # This looks like normalize instead of converting to 0/1
        if self.binarize:
...
class BCosAttributor(AttributorBase):
    ...
    def _call_single(self, feature, output, class_idx, img_idx):
        ...
        attributions = (
            (grads[img_idx] * feature[img_idx]).sum(dim=0, keepdim=True).unsqueeze(0)
        )
        return self.apply_post_processing(attributions)
...
class GradCamAttributor(AttributorBase):
    ...
    def _call_batch_mode(self, feature, output, classes):
        target_outputs = torch.gather(output, 1, classes.unsqueeze(-1))
        grads = torch.autograd.grad(
            torch.unbind(target_outputs), feature, create_graph=True, retain_graph=True
        )[0]
        grads = grads.mean(dim=(2, 3), keepdim=True)
        prods = grads * feature
        attributions = torch.nn.functional.relu(torch.sum(prods, axis=1, keepdim=True))
        return self.apply_post_processing(attributions)

    def _call_single(self, feature, output, class_idx, img_idx):
        grads = torch.autograd.grad(
            output[img_idx, class_idx], feature, create_graph=True, retain_graph=True
        )[0]
        grads = grads.mean(dim=(2, 3), keepdim=True)
        prods = grads[img_idx] * feature[img_idx]
        attributions = torch.nn.functional.relu(
            torch.sum(prods, axis=0, keepdim=True)
        ).unsqueeze(0)
        return self.apply_post_processing(attributions)
